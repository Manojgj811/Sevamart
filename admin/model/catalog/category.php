...
class ModelCatalogCategory extends Model {
	...
	///////////////////   to display categories for store manger based on Id's  getCategorysByStoreId($data = array() ,$storeid) 
			public function getCategorysByStoreId($data = array() ,$storeid   )    
			{

				$query = ("SELECT c1.category_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;'  ) AS name, c1.parent_id, c1.sort_order 
				...
				LEFT JOIN " . DB_PREFIX . "category_to_store cs ON (c1.category_id = cs.category_id)
				
				WHERE cs.store_id = '" . (int)$storeid . "'   ");

                if(!empty($data['filter_name'])) 
			     {
					$query .= " AND cd2.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
				
				  }

				  $query .= " GROUP BY c1.category_id";
  ...
  ///////////////////////////////get total categories for  individual storemanagers  to view to categories created by sharath on 14/05/2021


	public function getTotalCategoriesByStoreId($storeid) 
	{
	///	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category");
		
	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_to_store cs ON (c.category_id = cs.category_id)
			LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id)  WHERE cs.store_id = '" . (int)$storeid . "'");
