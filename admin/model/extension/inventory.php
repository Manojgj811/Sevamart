...
// Error Handler
function error_handler_for_inventory($errno, $errstr, $errfile, $errline) {
	global $registry;
...
function fatal_error_shutdown_handler_for_inventory()
{
	$last_error = error_get_last();
	if (($last_error) && ($last_error['type'] === E_ERROR)) {
		// fatal error
		error_handler_for_inventory(E_ERROR, $last_error['message'], $last_error['file'], $last_error['line']);
	}
}


class ModelToolInventory extends Model
 {
	private $error = array();
	...
	protected function storeProductIntoDatabase( &$product ) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET  sku = '" .$sku. "', quantity='" .$quantity . "', 
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array(),$current_store_id ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$this->storeProductIntoDatabase( $product );
		}
	}
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	public function upload( $filename, $current_store_id, $incremental=false ) {
		// we use our own error handler
		global $registry;
		$registry = $this->registry;
		set_error_handler('error_handler_for_inventory',E_ALL);
		register_shutdown_function('fatal_error_shutdown_handler_for_inventory');
...
if (version_compare(VERSION,'3.0','>=')) {
	class ModelExtensionInventory extends ModelToolInventory {
	}
}
