...
private $error = array();
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids )
	 {
		...
		$sql  = "INSERT INTO `".DB_PREFIX."product` (`quantity`,`sku`,`upc`,";
	...
	protected function deleteProduct( $sku, $product_id, $exist_table_product_tag ) {
		$sql  = "DELETE FROM `".DB_PREFIX."product` WHERE `sku` = '$sku';\n";
	...
	// protected function deleteProductBasedonsku( $product_id, $exist_table_product_tag ) {
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array(), $current_store_id ) {
		// get worksheet, if not there return immediately
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			if ($incremental)
			{
				$this->deleteProduct( $sku, $product_id, $exist_table_product_tag );
			}
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function validateSkuIdColumns( &$reader ) {
		$data = $reader->getSheetByName( 'Products' );
		if ($data==null) {
			return true;
		}
		...
		// only unique numeric product_ids can be used, in ascending order, in worksheet 'Products'
		$previous_sku_id = 0;
		$has_missing_sku_ids = false;
		$sku_ids = array();
		$k = $data->getHighestRow();
		for ($i=1; $i<$k; $i+=1) {
			$sku_id = $this->getCell($data,$i,4);
			if ($sku_id=="") {
				if (!$has_missing_sku_ids) {
					$msg = str_replace( '%1', 'Products', $this->language->get( 'error_missing_sku_id' ) );
					$this->log->write( $msg );
					$has_missing_aku_ids = true;
				}
				$ok = false;
				continue;
			}
			if (!$this->isAlphaNumeric($sku_id)) {
				$msg = str_replace( '%2', $sku_id, str_replace( '%1', 'Products', $this->language->get( 'error_invalid_sku_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
				continue;
			}
			if (in_array( $sku_id, $sku_ids )) {
				$msg = str_replace( '%2', $sku_id, str_replace( '%1', 'Products', $this->language->get( 'error_duplicate_sku_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$sku_ids[] = $sku_id;
			// if ($sku_id < $previous_sku_id) {
			// 	$msg = str_replace( '%2', $product_id, str_replace( '%1', 'Products', $this->language->get( 'error_wrong_order_sku_id' ) ) );
			// 	$this->log->write( $msg );
			// 	$ok = false;
			// }
			// $previous_sku_id = $sku_id;
		}
	...
	protected function isAlphaNumeric($input){
		return(ctype_alnum(strval($input)));
	}
	...
	protected function validateUpload( &$reader )
	{
		...
		if (!$this->validateSkuIdColumns( $reader )) {
			$ok = false;
		}
	...
	public function upload( $filename, $incremental=false, $current_store_id) {
		...
		try {
			...
			$this->uploadProducts( $reader, $incremental, $available_product_ids, $current_store_id );
	...
	protected function getProductDescriptions( &$languages, $offset=null, $rows=null, $min_id=null, $max_id=null, $current_store_id ) {
		// some older versions of OpenCart use the 'product_tag' table
		$exist_table_product_tag = false;
		$query = $this->db->query( "SHOW TABLES LIKE '".DB_PREFIX."product_tag'" );
		$exist_table_product_tag = ($query->num_rows > 0);
		...
		foreach ($languages as $language) {
			...
			if (true) {
				$sql .= "WHERE ps.store_id=".$current_store_id." ";
				if ($this->posted_categories) {
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $exist_seo_url_table, $offset=null, $rows=null, $min_id=null, $max_id=null, $current_store_id ) {
		$sql  = "SELECT ";
		$sql .= "  p.product_id,";
		...
		$results = $this->db->query( $sql );
		$product_descriptions = $this->getProductDescriptions($languages, $offset, $rows, $min_id, $max_id, $current_store_id );
		foreach ($languages as $language) {
			$language_code = $language['code'];
			foreach ($results->rows as $key=>$row) {
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null, $current_store_id) {
		// get list of the field names, some are only available for certain OpenCart versions
		$query = $this->db->query( "DESCRIBE `".DB_PREFIX."product`" );
		$product_fields = array();
		foreach ($query->rows as $row) {
		...
		$products = $this->getProducts( $languages, $default_language_id, $product_fields, $exist_meta_title, $exist_seo_url_table, $offset, $rows, $min_id, $max_id, $current_store_id );
		$len = count($products);
	...
	protected function getAdditionalImages( $min_id=null, $max_id=null, $exist_sort_order=true, $current_store_id  ) {
		if ($exist_sort_order) {
		...
		$sql .= "LEFT JOIN `".DB_PREFIX."product_to_store` ps ON ps.product_id=pi.product_id ";
	...
	protected function populateAdditionalImagesWorksheet( &$worksheet, &$box_format, &$text_format, $min_id=null, $max_id=null, $current_store_id) {
		// check for the existence of product_image.sort_order field
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."product_image` LIKE 'sort_order'";
		$query = $this->db->query( $sql );
		$exist_sort_order = ($query->num_rows > 0) ? true : false;
		...
		$i += 1;
		$j = 1;
		$additional_images = $this->getAdditionalImages( $min_id, $max_id, $exist_sort_order, $current_store_id );
	...
	public function download( $export_type, $offset=null, $rows=null, $min_id=null, $max_id=null, $current_store_id) {
		// we use our own error handler
		global $registry;
		$registry = $this->registry;
		set_error_handler('error_handler_for_export_import',E_ALL);
		...
		try {
			...
			///to hide
			$workbook->getActiveSheet()->getColumnDimension('A')->setVisible(false);
			$workbook->getActiveSheet()->getColumnDimension('C')->setVisible(false);
			$workbook->getActiveSheet()->getColumnDimension('AH')->setVisible(false);
			$workbook->getActiveSheet()->getColumnDimension('Aj')->setVisible(false);
			...
			switch ($export_type) {
				...
				case 'p':
					...
					$this->populateProductsWorksheet( $worksheet, $languages, $default_language_id, $price_format, $box_format, $weight_format, $text_format, $offset, $rows, $min_id, $max_id, $current_store_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
					...
					// $this->populateAdditionalImagesWorksheet( $worksheet, $box_format, $text_format, $min_id, $max_id, $current_store_id );
					// $worksheet->freezePaneByColumnAndRow( 2, 2 );
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("https://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_4_9&language_code=$language_code");
