...
class ModelExtensionReportSale extends Model {
	...
	{
	// 	$sql =	"SELECT o.order_id, o.invoice_no , o.store_id , o.date_added AS date_start,o.total AS totalamount ,
	// 	GROUP_CONCAT(concat(ot.value )) As tax,  GROUP_CONCAT(concat(ot.value )) As Secondtax,
	//    SUM(ot.value) AS total
	//    FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id)
	//    WHERE ot.code = 'tax' ";
				
		$sql =	"SELECT o.order_id, o.invoice_no ,o.invoice_prefix, o.store_id , o.date_added AS date_start,o.total AS totalamount ,
        otax.cgst_order_amount  AS tax ,otax.sgst_order_amount  As Secondtax,   
		SUM(ot.value) AS total
		FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id)
        LEFT JOIN oc_order_tax otax ON (otax.order_id = o.order_id) 
...
//// created by sharath on 23-04-2021  =>to display taxes based on individual store for store managers.
	public function getTaxesbystore($data = array(),$storeid) 
	{
	//	$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, o.order_id, o.invoice_no , o.store_id ,ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id) 
		//WHERE ot.code = 'tax'  and o.store_id= $storeid ";COUNT(o.order_id) AS `orders` 
	 $sql = "SELECT o.order_id, o.invoice_no ,o.invoice_prefix, o.store_id ,o.date_added AS date_start, o.total AS totalamount , GROUP_CONCAT(concat(ot.value )) As taxes,
	SUM(ot.value) AS total  ,   otax.cgst_order_amount  AS tax ,otax.sgst_order_amount  As Secondtax
	FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id)

    LEFT JOIN oc_order_tax otax ON (otax.order_id = o.order_id)

	WHERE ot.code = 'tax' and o.store_id= $storeid    ";
	...
	public function subtotalstore($storeid)
	{
		
		$sql2="SELECT ot.value as orderamount
		
		FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id) 
	   WHERE ot.code = 'sub_total'  and o.store_id= $storeid  ";
