<?php
class ModelExtensionReportCommissionPercentage extends Model {
	...
	public function getOrders($data = array()) {
		...
		o.store_id,
		 SUM((SELECT SUM(op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id)) AS products FROM `" . DB_PREFIX . "order` o";

		// SUM((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o";
	...
	public function getOrdersbystore( $data = array() , $storeid) 
	{
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, COUNT(*) AS `orders`,SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o
	...
	public function getCommission($data = array()) 
	{
		...
		$sql =	"SELECT o.order_id, o.invoice_no ,o.invoice_prefix, o.date_added AS date_start,o.store_id , o.payment_method AS Mode_of_payment, ot.sub_total,  ot.cgst_order_amount,  ot.sgst_order_amount, ot.delivery_charge, ot.cgst_delivery_percentage, ot.cgst_delivery_amount, ot.sgst_delivery_percentage, ot.sgst_delivery_amount, ot.commission_percentage, ot.commission_amount, ot.cgst_commission_percentage, ot.cgst_commission_amount, ot.sgst_commission_percentage, ot.sgst_commission_amount, ot.total_commission_amount FROM `oc_order` o inner JOIN `oc_order_tax` ot ON ot.order_id = o.order_id ";
...
//// created by sharath on 23-04-2021  =>to display taxes based on individual store for store managers.
	public function getCommissionbystore($data = array(),$storeid) 
	{
	
		$sql = "SELECT o.order_id, o.invoice_no ,o.invoice_prefix, o.date_added AS date_start,o.store_id ,
		 o.payment_method AS Mode_of_payment, ot.sub_total, ot.cgst_order_amount, 
		 ot.sgst_order_amount, ot.delivery_charge, ot.cgst_delivery_percentage,
		  ot.cgst_delivery_amount, ot.sgst_delivery_percentage, ot.sgst_delivery_amount,
		   ot.commission_percentage, ot.commission_amount, ot.cgst_commission_percentage,
		    ot.cgst_commission_amount, ot.sgst_commission_percentage, ot.sgst_commission_amount, ot.total_commission_amount
			 FROM `oc_order` o inner JOIN `oc_order_tax` ot ON (ot.order_id = o.order_id ) WHERE o.store_id= $storeid    ";
