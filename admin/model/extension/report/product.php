...
class ModelExtensionReportProduct extends Model {
	...
	public function getProductsViewedbystore($storeid) 
	{
	$sql = "SELECT pd.name, p.model, p.viewed 
	...
	public function getPurchased($data = array()) {
		$sql = "SELECT op.name, op.model, op.product_id,  o.store_id , SUM(op.quantity) AS quantity, SUM((op.price + op.tax) * op.quantity) AS total 
		...
		$sql .= " GROUP BY op.order_id ORDER BY total DESC";
	...
	public function getPurchasedbasedonstore($data,$storeid)
	 {
		$sql = "SELECT op.name, op.model,op.product_id,  o.store_name, (op.quantity) AS quantity, SUM((op.price + op.tax) * op.quantity) AS total
		 FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) 
