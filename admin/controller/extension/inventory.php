<?php
class ControllerExtensionInventory extends Controller { 
	private $error = array();
	private $ssl = 'SSL';
	...
	{
		$this->load->language('extension/inventory');
		$this->document->setTitle($this->language->get('heading_title'));
	//model_extension_export_import
	$this->load->model('extension/export_import');
		$this->load->model('extension/inventory');
		$this->getForm();
	}
	...
	public function upload()
	 {
		...
		if($user_info)
		{
                 $current_id['storeid']=$user_info['store_id'];
				 $current_store_id=$current_id['storeid'];
   
				 if (isset($this->request->post['adminstoreids'])) 
				 {
				   $data['storeid'] = $this->request->post['adminstoreids'];
				 } 
   
					
				 if($current_store_id == 0 )
				 {
				   $current_store_id = (int)$data['storeid'];
				 }
			...
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name'])))
			 {
				...
				if ($this->model_extension_inventory->upload($file,$current_store_id,true)==true) {
					
					$this->session->data['success'] = $this->language->get('text_success');
					$this->response->redirect($this->url->link('extension/inventory', 'user_token=' . $this->session->data['user_token'], $this->ssl));
				}
				else {
	...
	public function download() 
	{
		...
		if($user_info)
		{
				 ...
				 if (isset($this->request->post['adminstoreid'])) 
				 {
				   $data['storeid'] = $this->request->post['adminstoreid'];
				 } 
					...
					switch ($export_type) 
					{
						...
						case 'u':
							...
							if (($min==null) || ($max==null)) {
								$this->model_extension_inventory->download($export_type, null, null, null, null, $current_store_id);
							} else if ($this->request->post['range_type'] == 'id') {
								$this->model_extension_inventory->download($export_type, null, null, $min, $max);
							} else {
								$this->model_extension_inventory->download($export_type, $min*($max-1-1), $min, null, null);
							}
							break;
						case 'o':
							$this->model_extension_inventory->download('o', null, null, null, null);
							break;
						case 'a':
							$this->model_extension_inventory->download('a', null, null, null, null);
							break;
						case 'f':
							if ($this->model_extension_inventory->existFilter()) {
								$this->model_extension_inventory->download('f', null, null, null, null);
								break;
							}
							break;
						default:
			...
			$this->response->redirect( $this->url->link( 'extension/inventory', 'user_token='.$this->request->get['user_token'], $this->ssl) );
		}
	  }
		$this->getForm();
	}
	...
	protected function getForm() {
		...
		$data['exist_filter'] = $this->model_extension_inventory->existFilter();
		...
		$data['button_inventory_export'] = $this->language->get( 'button_inventory_export' );
		$data['button_inventory_import'] = $this->language->get( 'button_inventory_import' );
		$data['button_settings'] = $this->language->get( 'button_settings' );
		////changed for inventory
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/inventory', 'user_token=' . $this->session->data['user_token'], $this->ssl)
		);

		$data['back'] = $this->url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['button_back'] = $this->language->get( 'button_back' );
		$data['import'] = $this->url->link('extension/inventory/upload', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['export'] = $this->url->link('extension/inventory/download', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['settings'] = $this->url->link('extension/inventory/settings', 'user_token=' . $this->session->data['user_token'], $this->ssl);
...
/////////////////////for dropdown for super admins
		...
		$this->response->setOutput($this->load->view( 'extension/inventory', $data));
	}
