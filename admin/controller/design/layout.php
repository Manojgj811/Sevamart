...
class ControllerDesignLayout extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_design_layout->addLayout($this->request->post, $data['store_id']);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if($result['store_id'] == $data['store_id']){
			$data['layouts'][] = array(
	...
	protected function getForm() {
		...
		$user_info = $this->model_user_user->getUserstore($this->user->getId());



		if (isset($this->request->post['store'])) {
			$data['store'] = $this->request->post['store'];
		} elseif (isset($user_info['store'])) {
			$data['store'] = $user_info['store'];
		} else {
			$data['store'] = '';
		}
		...
		foreach ($extensions as $code) {
			...
			$modules = $this->model_setting_module->getModulesByCode($code,$data['store_id']);
		...
		foreach ($layout_modules as $layout_module) {
			...
			if (!isset($part[1])) {
				...
				$module_info = $this->model_setting_module->getModule($part[1],$data['store_id']);
