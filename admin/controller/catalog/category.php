	...
	protected function getList() {
		...
		$user_info = $this->model_user_user->getUser($this->user->getId());
         ...
         if($user_info )
           ...
           /////////////////// for storemanagers created by sharath on 14/05/2021
	if($user_info )
		{
			$data['user_group'] = $user_info['user_group_id'];
			$data['storeid']=$user_info['store_id'];
				...
				foreach($storeresults  as $storeids)
					{
						...
						if($data['storeid']==$sid )
						{
							$storecategorytotal = $this->model_catalog_category->getTotalCategoriesByStoreId($sid);
							//  var_dump(	$storecategorytotal);
							$categoryresults= $this->model_catalog_category->getCategorysByStoreId( ' ', $sid);
...
$data['text_form']=!isset($this->request->get['category_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		$data['user_group'] = $user_info['user_group_id'];
			...
			if( $user_info['user_group_id'] != 1  )
			{
			 ...
			 if( $user_info['user_group_id'] == 1  )
			 {
				 if (isset($this->request->post['path2'])) {
				 $data['path'] = $this->request->post['path2'];
			 } elseif (!empty($category_info)) {
				 $data['path'] = $category_info['path'];
			 } else {
				 $data['path'] = '';
		...
		*/
		...
		$usergroup=$this->model_user_user_group->getUserGroups();

		//var_dump($usergroup[0]);
...
/////////  autocomplete textbox for store manager
	public function autocompleteStoreManager() {
		$json = array();
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$getCategoriesByStore = $this->model_catalog_category->getCategorysByStoreId($filter_data , $user_info['store_id'] );

			foreach ($getCategoriesByStore as $result) 
			{
				//if( $user_info['user_group_id'] !=1  )
