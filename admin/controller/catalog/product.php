		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $result['product_id'] . $url, true)
                    ...
                    ///////////////for storemanagers created  by sharath on 13/05/2021
				...
				if($user_info )
					{
                       $data['user_group'] = $user_info['user_group_id'];
						...
						foreach($storeresults  as $storeids)
							{
								$sid=$storeids['store_id'];
							
				            	if($data['storeid']==$sid )
                             {
                 	
								$results= $this->model_catalog_product->getProductsByStoreId($filter_data ,$sid);
				...
				foreach ($results as $result) {
					...
					$data['storemanagerproducts'][] = array(
						'product_id' => $result['product_id'],
						'image'      => $image,
						'name'       => $result['name'],
						'model'      => $result['model'],
...
/////////////// created for sku
		...
		if (isset($this->error['tax_class_id'])) {
			$data['error_tax_class_id'] = $this->error['tax_class_id'];
		} else {
			$data['error_tax_class_id'] = '';
		}
		...
		if ($user_info) 
		{
			...
			$data['usergroupid']=$user_info['user_group_id'];

		  if($data['usergroupid']!=1 ||	$data['usergroupid']==1)
			 {
	//	echo $data['storeid'];
		...
		*/
		// commented to hide  the defaultstore
	   
     //only for assigned stores ,store manager can add products.
      $this->load->model('user/user');
         $user_info = $this->model_user_user->getUser($this->user->getId());
		 ...
		 if ($user_info) 
		 {
           ...
           foreach ($stores as $store) 
		   {
			if($data['usergroupid']==1)
			  ...
			  if($data['storeid']==$store['store_id']   &&    $data['usergroupid']!=1  )
		          {
			$data['stores'][] = array(
				'store_id' => $store['store_id'],
				'name'     => $store['name']
		...
		///
		if (isset($this->request->post['storemangerstore'])) {
			$data['storemangerstore'] = $this->request->post['storemangerstore'];
			//echo "yes passed";
		} elseif (isset($this->request->get['product_id'])) {
			$data['storemangerstore'] = $this->model_catalog_product->getProductStores($this->request->get['product_id']);
		} else {
			$data['storemangerstore'] = array(0);
		}
		...
		foreach ($categories as $category_id) {
           ...
           /////  Categories for admin$this->load->model('user/user');
		...
		if($user_info )
		   {
			$data['user_group'] = $user_info['user_group_id'];
			   ...
			   { 
		     	      ...
		     	      if (isset($this->request->post['product_category']))
					{
						$categories2 = $this->request->post['product_category'];
					}
					elseif (isset($this->request->get['product_id'])) 
					{
						$categories2 = $this->model_catalog_product->getProductCategories($this->request->get['product_id']);
					} 
					else 
					{
					$categories2 = array();
				...
				foreach ($categories2 as $category_id) {
					$category_info = $this->model_catalog_category->getCategory($category_id);
					//var_dump($category_info);
		...
		if( $user_info['user_group_id'] == 1  )
		{
		 ...
		 //  var_dump( $data['layouts']);

		}

		if( $user_info['user_group_id'] !=1  )
		{
			$data['layouts'] = $this->model_design_layout->getLayoutsBasedOnStore($user_info['store_id']);	
	...
	protected function validateForm() {
		...
		{
			$this->error['tax_class_id'] = 'please select tax class';
		}
		...
		if ((utf8_strlen($this->request->post['sku']) < 1) || (utf8_strlen($this->request->post['sku']) > 64)) {
			//$this->error['sku'] = 'please fill sku';
