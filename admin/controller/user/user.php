...
/*
		if (!isset($this->request->get['store_id'])) {
			...
			$data['action'] = $this->url->link('user/user/edit', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $this->request->get['store_id'] . $url, true);
		}
*/
		...
		if (isset($this->request->get['user_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$store_info = $this->model_user_user->getUserstore($this->request->get['user_id']);
		}
	   ...
	   {
			$data['store_id'] = $this->request->post['storeid'];
		} 
		...
		$data['storeids'] = $this->model_setting_store->getStores();
...
/////
		...
		if (isset($this->request->post['mobilenumber']))
		 ...
		 elseif (!empty($user_info)) {
			$data['mobilenumber'] = $user_info['mobilenumber'];
		} 
		else {
			$data['mobilenumber'] = '';
		}
