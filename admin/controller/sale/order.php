...
////changes done on 05-04-2021 for dropdown
    ...
    ///from 254 line to 268 line for dropdown implementation
      ...
      //var_dump($user_info);
				...
				{
              $results2 = $this->model_sale_order->getTotalOrdersByStoreId($storeid);
		//var_dump($results2);
		...
		//$this->currency->format( $unit_price * $product['quantity'], $this->session->data['currency']);
				...
				$data['orders2'][] = array(
					...
					'total'         => $this->currency->format($orderresult['total'],   $this->session->data['currency']  ),
					...
					'view'          => $this->url->link('sale/order/info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $orderresult['order_id'] . $url, true),
					'edit'          => $this->url->link('sale/order/edit', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $orderresult['order_id'] . $url, true)
				);
				}
				}
			}
	...
	public function getForm() {
      ...
      // var_dump($products);
			...
			//var_dump($product['deliverycharge']);
			// Vouchers
			...
			foreach ($order_totals as $order_total) {
               ...
               ///////////////////// to display delivery rate of order in editing order

			   $ordertaxproductsedit=$this->model_sale_order->getOrderTax($this->request->get['order_id']);

			   // var_dump( $ordertaxproducts);
   
				foreach($ordertaxproductsedit  as $taxvalue)
				{  
					$data['shippingedit'][]=array(
				   'title'=>'Shipping Charges(ex GST):',
   
				   'sumdeliverytotal'=>  $this->currency->format ($taxvalue['delivery_charge'],$this->session->data['currency']),
			   			   
			   );
			 $mmmoo=  $taxvalue['delivery_charge'];
		         }
   
		  //  echo "the money of ordershipping is $mmmoo";
	...
	public function info() {
       ...
       $this->load->model('setting/setting');
		...
		$orderpickup=$order_info['pickup'];
 		...
 		$orderdelivery=$order_info['delivery'];

		 $orderdmtd	=$order_info['order_method'];
		

	// 	 echo " the pickup is 	$orderpickup";

	// 	 echo "<br>";

	// echo " the deliver val is  is $orderdelivery";
     ...
     //	var_dump($ordertotalamount);
		...
		$data['store_name'] = $order_info['store_name'];
				...
				if(	$order_info['order_method']==" POS ")
				{
					$data['order_method'] ="Walk-IN";
				
				}

				else
				{
				$data['order_method'] = "Online ";
...
////
			...
			$data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
                ...
                $get=$data['payment_address'];

				$ad=gettype( $get);
				
				// echo "the type of variable is $ad";
 ...
 //////////////////////////
		...
		//	echo "the sum of  cgsttotalamount is $rate_sum";
			...
			foreach ($vouchers as $voucher) {
                ...
                /// $order pickup value is one , then dont display shipping delivery details
	...
	// 		'cgsttitle'=>'CGST Amount on shipping(@'. $taxvalue['cgst_delivery_percentage'] .'%):',
	// 		'sumdeliverytotal'=>  $this->currency->format ((float)$deliverybillamount,$this->session->data['currency']),
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			$ordermethod=$order_info['order_method'];

			//var_dump($ordermethod);	var_dump($ordertotalamount);
	   ...
	   if ($order_info) {
		...
		$data['GrandTotalForshipping'][]=array(
			'title'=>'GrandTotal(INC GST)',
			'text'=>  $this->currency->format($ordertotalamount,$this->config->get('config_currency'))
				 );
		   ...
		   'cgsttitle'=>'CGST Amount on shipping(@'. $taxvalue['cgst_delivery_percentage'] .'%):',
		   'sumdeliverytotal'=>  $this->currency->format ((float)$deliverybillamount,$this->config->get('config_currency')),
		   'cgstsumdeliverytotal'=>  $this->currency->format( $taxvalue['cgst_delivery_amount'],$this->config->get('config_currency')),
		   'sgsttitle'=>'SGST Amount on shipping(@'. $taxvalue['sgst_delivery_percentage'] .'%):',
		   'sgstsumdeliverytotal'=>  $this->currency->format($taxvalue['sgst_delivery_amount'],$this->config->get('config_currency'))
		...
		$data['cgsttotalsum'][]=array(
			'title'=>'CGST Amount',
			'cgstvalue'=>   $this->currency->format($rate_sum+@$taxvalue['cgst_delivery_amount'],$this->config->get('config_currency')    )
				 );
		...
		$data['sgsttotalsum'][]=array(
			'title'=>'SGST Amount',
			'sgstvalue'=>   $this->currency->format($sgst+@$taxvalue['sgst_delivery_amount'],$this->config->get('config_currency')    )
				 );
			...
			//	var_dump($data['GrandTotalForshipping']);
				...
				$data['orders'][] = array(
					...
					'order_method'   => $order_info['order_method'],
