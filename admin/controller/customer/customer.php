	...
	protected function getList() {
           ...
           $this->load->model('setting/store');
			...
			$results = $this->model_setting_store->getStores();
             ...
             if($data['user_group']==1)
             {
				...
				'select'=> $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] .'&code=' . $code['store_id'] , true )
				);
			//	echo $code['store_id'];
			   }
		...
		$results = $this->model_setting_store->getStores();
				...
				$this->load->model('customer/customer');
					...
					$storemangerstores = $this->model_customer_customer->getTotalCustomersofstoredetails($storeid);
			
				}
		foreach ($storemangerstores as  $dataresult)
	        	{
			$data['dropdownstores2'][] = array(
				   'customer_id'    => $dataresult['customer_id'],
						'name'           => $dataresult['firstname'],
						'email'          => $dataresult['email'],
						'customer_group' => $dataresult['customer_group_id'],
						'status'         => ($dataresult['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')),
						'ip'             => $dataresult['ip'],
						'date_added'     => date($this->language->get('date_format_short'), strtotime($dataresult['date_added'])),
						'unlock'         => $unlock,
						'store'          => $store_data,
						'edit'           => $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $dataresult['customer_id'] . $url, true)
		...
		$customer_total=$this->model_customer_customer->getTotalCustomers($filter_data);
         ...
         $this->load->model('user/user');
			...
			$data['user_group'] = $user_info['user_group_id'];
				...
				{
				$customerstoreresults = $this->model_customer_customer->getTotalCustomersofstoredetails($storeid);
			//	var_dump($customerstoreresults);
				}
			}
			   
			foreach ($customerstoreresults as $dataresult) {
					$login_info = $this->model_customer_customer->getTotalLoginAttempts($dataresult['email']);

					if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
						$unlock = $this->url->link('customer/customer/unlock', 'user_token=' . $this->session->data['user_token'] . '&email=' . $dataresult['email'] . $url, true);
					} else {
						$unlock = '';
					}
					...
					$store_data[] = array(
						...
						'href' => $this->url->link('customer/customer/login', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $dataresult['customer_id'] . '&store_id=0', true)
					);

					foreach ($stores as $store) {
						$store_data[] = array(
							'name' => $store['name'],
							'href' => $this->url->link('customer/customer/login', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $dataresult['customer_id'] . '&store_id=' . $dataresult['store_id'], true)
						);
					}
					
					$data['customersbasedonstore'][] = array(
						'customer_id'    => $dataresult['customer_id'],
						'name'           => $dataresult['firstname'],
						'email'          => $dataresult['email'],
						'customer_group' => $dataresult['customer_group_id'],
