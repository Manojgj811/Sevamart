...
class ControllerProductSearch extends Controller {
	...
	/////////start to fetch login user /  login storemanager storeID
	...
	{ 
		...
		if (isset($this->request->get['search'])) {
			$this->document->setTitle('search' .  ' - ' . $this->request->get['search']);
		} 
		...
		if (isset($this->request->get['search'])) 
		{
			//echo "yes if this one only";

			$data['heading_title'] = 'search '.  ' - ' . $this->request->get['search'];
		}
	 else 
		 {
		...
		$categories_1 = $this->model_sale_poscategory->getCategories(0,	$storemanager_storeID);


	 //	$categories_1 = $this->model_catalog_category->getCategories(0);$this->model_catalog_category->getCategories($category_1['category_id']);

		foreach ($categories_1 as $category_1) {
			$level_2_data = array();

			$this->model_sale_poscategory->getCategories(0,	$storemanager_storeID);

			$categories_2 = $this->model_sale_poscategory->getCategories($category_1['category_id'],$storemanager_storeID);

			foreach ($categories_2 as $category_2) {
				$level_3_data = array();

				$categories_3 = $this->model_sale_poscategory->getCategories($category_2['category_id'],$storemanager_storeID);
		...
		//	$product_total = $this->model_catalog_product->getTotalProducts($filter_data);	//$results = $this->model_catalog_product->getProducts($filter_data);

        $product_total =$this->model_sale_poscategory->getTotalProducts($filter_data,$storemanager_storeID );

		$results = $this->model_sale_poscategory->getProducts($filter_data , $storemanager_storeID);
	...
	$data['key'][]=$allKeys[0];
		...
		//	var_dump($limit);
			...
			$pagination->url = $this->url->link('product/search&user_token='.$this->session->data['user_token'], $url . '&page={page}');
		...
		///// pos controller
     	$data['possearch'] = $this->load->controller('sale/possearch');

		 $data['posmenu'] = $this->load->controller('sale/posmenu');
