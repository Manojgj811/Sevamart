...
class ModelLocalisationCancelReason extends Model {
	public function getCancelReasons($data = array()) {
		if ($data) {
			...
			$return_reason_data = $this->cache->get('cancel_reason.' . (int)$this->config->get('config_language_id'));
			...
			if (!$return_reason_data) {
				...
				$this->cache->set('cancel_reason.' . (int)$this->config->get('config_language_id'), $return_reason_data);
			}
