<?php
class ModelAccountCancel extends Model {
	...
	public function getRequestedDate($order_id, $product_id){
		...
		$query = $this->db->query("SELECT date_added , request_type FROM `" . DB_PREFIX . "cancel` WHERE order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$product_id . "' ");
	...
	public function addCancel($data) {
		...
		$config_cancel_status_id = 3;
		$this->db->query("INSERT INTO `" . DB_PREFIX . "cancel` SET order_id = '" . (int)$data['order_id'] . "', product_id = '" . (int)$data['product_id'] . "', customer_id = '" . (int)$this->customer->getId() . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', product = '" . $this->db->escape($data['product']) . "', model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', cancel_reason_id = '" . (int)$data['cancel_reason_id'] . "', cancel_status_id = '" . (int)$config_cancel_status_id . "', request_type = '" . $this->db->escape($data['request_type']) . "' , date_ordered = '" . $this->db->escape($data['date_ordered']) . "', date_added = NOW(), date_modified = NOW()");
