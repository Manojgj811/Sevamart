...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				$this->model_checkout_order->changeOrderStatus($order_id, $this->request->post['order_status_id']);
				//var_dump($this->request->get['email']);
				//$this->sendMail($this->request->get['email']);
				$json['success'] = $this->language->get('text_success');
			} else {
				$json['error'] = $this->language->get('error_not_found');
			}
	...
	public function sendMail($emailId){
		...
		$mail->setText("Your order has been delivered.");
		$mail->send();
