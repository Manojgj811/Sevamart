<?php
class ControllerAccountItemNotReceived extends Controller {
	private $error = array();

	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/item_not_received', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/item_not_received', $url, true)
		);
		  ...
		  if($result['status'] == "Delivered"){
			...
			$products = $this->model_account_order->getOrderProducts($result['order_id']);
		     ...
		     foreach ($products as $product){
				...
				if($product_status_id == 4){

					$requestedDate = $this->model_account_item_not_received->getRequestedDate($result['order_id'], $product['product_id']);
				}
				else{
		...
		$newArray = array_filter($data['productsList'], function ($var) {
			return ($var['status'] == 'Delivered' and ($var['product_status'] == "7" or $var['product_status'] == "4") );
		});
		...
		$data['action'] = $this->url->link('account/item_not_received/add', '', true);
		...
		$this->response->setOutput($this->load->view('account/item_not_received_list', $data));
	}
	...
	public function add() {
		...
		$this->response->redirect($this->url->link('account/item_not_received', 'error='.$this->error["form"] , true));
	...
	public function success() {
		...
		$this->response->setOutput($this->load->view('common/success_item_not_received', $data));
	}
	...
	public function sendMail($emailId, $product_name){
		...
		$mail->setText($product_name . "has been requested for Item Not Received.");
		$mail->send();
