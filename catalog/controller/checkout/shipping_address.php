...
class ControllerCheckoutShippingAddress extends Controller {
			...
			// echo $addresspostcode;
			   ...
			   if ($addresspostcode==$pincodevalue)
				 { 
				   ...
				   $data['amounts'][] = array(
					'name' => $this->currency->format($output , $this->session->data['currency'])
				       );
	...
	public function save() {
		...
		if (!$json) {
			...
			$this->load->model('account/address');
			unset($this->session->data['shipping_pickup']);
			unset($this->session->data['shipping_delivery']);
		
			if (isset($this->request->post['shipping_type']) && $this->request->post['shipping_type'] == 'pickByStore') {

				
				if (!$json) {

				//	$check_address_id = $this->model_account_address->checkAddress($this->customer->getId());
              // var_dump($check_address_id);
					//$address_id = $this->model_account_address->addAddress($this->customer->getId(), $this->request->post);

				//	$this->session->data['shipping_address'] = $this->model_account_address->getAddress($address_id);
                    $this->session->data['shipping_pickup'] = true;
					$this->session->data['shipping_delivery'] = false;
			...
			if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'existing') {
				...
				if (!$json) { 
                    ...
                    $this->session->data['shipping_pickup'] = false;
					$this->session->data['shipping_delivery'] = true;
					unset($this->session->data['shipping_method']);
					unset($this->session->data['shipping_methods']);
				}
			} else {
