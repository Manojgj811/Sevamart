<?php
class ControllerCommonWishList1 extends Controller {
	...
	public function add() {
		$this->load->language('common/wishlist1');
...
//var_dump($store_info);
		if ($store_info) {
			if ($this->customer->isLogged()) {
				...
				// Edit customers cart
				$this->load->model('common/wishlist1');

			 $this->model_common_wishlist1->addWishlist($this->request->post['store_id']);
			 ...
			 $json['success'] = $this->language->get('text_success');
				...
				$json['total'] = sprintf($this->language->get('text_wishlist'), $this->model_common_wishlist1->getTotalWishlist());
			} else {
	...
	$results = $this->model_common_wishlist1->getWishlist();
	...
	public function delete() {
		...
		if (isset($this->request->post['store_id'])) {
			//var_dump($this->request->post['store_id']);
			// Remove Wishlist
			$this->model_common_wishlist1->deleteWishlist($this->request->post['store_id']);
